...
const Financial: React.FC = () => {
  ...
  if (loading) return <div className="p-8 text-center text-[#666666]">Carregando financeiro...</div>;

  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-[#002A54] flex items-center gap-2">
          <Wallet className="text-[#009B4D]" /> Controle Financeiro
        </h1>
        <div className="text-right">
          <p className="text-xs text-[#666666] uppercase font-bold">Previsão (Mês Atual)</p>
          <p className="text-2xl font-bold text-[#002A54]">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h2 className="text-lg font-bold text-[#002A54] mb-6 flex items-center gap-2">
          <BarChart3 size={20} className="text-[#002A54]"/> Evolução de Receita
        </h2>
        <div className="h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <YAxis axisLine={false} tickLine={false} tickFormatter={(value) => `R$${value/1000}k`} tick={{fill: '#666666'}} />
              <Tooltip 
                cursor={{ fill: '#F8FAFC' }}
              ...
              {/* AZUL PARA FIXO, VERDE PARA COMISSÃO */}
              <Bar dataKey="Fixo" stackId="a" fill="#002A54" name="Receita Fixa" radius={[0, 0, 4, 4]} barSize={50} />
              <Bar dataKey="Comissao" stackId="a" fill="#009B4D" name="Comissões" radius={[4, 4, 0, 0]} barSize={50} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Card Fixo (Azul) */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 border-l-4 border-l-[#002A54]">
          <p className="text-sm font-medium text-[#666666]">Faturamento Fixo (Atual)</p>
          <p className="text-3xl font-bold text-[#002A54] mt-2">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 border-l-4 border-l-yellow-400">
          <p className="text-sm font-medium text-[#666666]">Comissões a Receber</p>
        ...
        {/* Card Recebido (Verde) */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 border-l-4 border-l-[#009B4D]">
          <p className="text-sm font-medium text-[#666666]">Comissões Recebidas</p>
          <p className="text-3xl font-bold text-[#009B4D] mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 lg:col-span-1 h-fit">
          <h2 className="text-lg font-bold text-[#002A54] mb-4 flex items-center gap-2">
            <TrendingUp size={20} className="text-[#002A54]"/> Contratos Mensais
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-white uppercase bg-[#002A54]">
                <tr><th className="px-3 py-2 rounded-tl-lg">Cliente</th><th className="px-3 py-2 text-right rounded-tr-lg">Valor</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {clients.map(client => (
                  <tr key={client.id} className={!client.active ? 'opacity-50 grayscale bg-slate-50' : ''}>
                    <td className="px-3 py-3 font-medium text-[#002A54]">
                      {client.company}
                      {!client.active && <span className="ml-2 text-[9px] bg-gray-200 px-1 rounded text-gray-600">PAUSADO</span>}
                    </td>
                    <td className="px-3 py-3 text-right text-[#666666] font-bold">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 lg:col-span-2">
          <h2 className="text-lg font-bold text-[#002A54] mb-4 flex items-center gap-2">
            <DollarSign size={20} className="text-[#009B4D]"/> Gestão de Comissões (Ganhas)
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-white uppercase bg-[#002A54]">
                <tr>
                  <th className="px-4 py-3 rounded-tl-lg">Licitação</th>
                  ...
                  return (
                    <tr key={bid.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="font-bold text-[#002A54]">{bid.title}</div>
                        <div className="text-xs text-[#666666]">{bid.client_name}</div>
                      </td>
                      <td className="px-4 py-3 text-right text-[#666666]">R$ {Number(bid.final_value).toLocaleString('pt-BR')}</td>
                      <td className="px-4 py-3 text-right">
                        <div className="font-bold text-[#009B4D]">R$ {commissionVal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <select 
                          ...
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold border outline-none cursor-pointer transition-colors ${
                            bid.financial_status === 'pago' ? 'bg-green-50 text-[#009B4D] border-green-200' : 
                            bid.financial_status === 'pendente' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : 'bg-slate-50 text-[#666666] border-slate-200'
                          }`}
                        >
