...
const Dashboard: React.FC = () => {
  ...
  if (loading) return <div className="p-8 flex justify-center text-slate-500">Carregando indicadores...</div>;
  ...
  const discardedBids = bids.filter(b => b.status === BidStatus.DISCARDED).length; // <--- NOVO CÁLCULO

  // 2. Taxa de Conversão (Win Rate)
  // Consideramos apenas o que foi disputado (Ganhas vs Perdidas). Descartadas não entram no mérito de "perda".
  const finishedBids = wonBids + lostBids;
  const winRate = finishedBids > 0 ? Math.round((wonBids / finishedBids) * 100) : 0;

  // 3. Prazos Críticos (Próximos 3 dias e que NÃO estão finalizadas)
  const criticalDeadlines = bids.filter(b => {
    if (!b.date) return false;
    const days = differenceInDays(parseISO(b.date), new Date());
    // Ignora o que já foi descartado/ganho/perdido
  ...
  // 4. Dados para Gráfico de Barras (Funil - Atualizado com Descartadas)
  const funnelData = [
    { name: 'Com Cliente', value: waitingClient, fill: '#3B82F6' }, // Azul
    { name: 'Ag. Licitação', value: waitingBidDay, fill: '#EAB308' }, // Amarelo
    { name: 'Descartadas', value: discardedBids, fill: '#94A3B8' }, // Cinza (Novo)
    { name: 'Ganhas', value: wonBids, fill: '#22C55E' }, // Verde
    { name: 'Perdidas', value: lostBids, fill: '#EF4444' }, // Vermelho
  ];

  // 5. Dados para Gráfico de Pizza (Top 5 Clientes Ativos)
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 mt-1">Acompanhe o desempenho da sua assessoria.</p>
        </div>
        <div className="text-right">
          <span className="text-sm font-medium text-slate-500">Taxa de Sucesso (Disputadas)</span>
          <div className="text-3xl font-bold text-green-600">{winRate}%</div>
        </div>
      </div>
      
      {/* KPI Cards (Agora são 5 colunas para caber o Descartadas, ou grid flexível) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        {/* Descartadas (NOVO) */}
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col justify-between border-b-4 border-b-gray-400 hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start mb-2">
            <p className="text-xs font-bold text-slate-400 uppercase">Descartadas</p>
            <div className="p-2 bg-gray-100 text-gray-500 rounded-lg"><Trash2 size={18} /></div>
          </div>
          <div>
            <p className="text-2xl font-bold text-gray-500">{discardedBids}</p>
            <p className="text-[10px] text-slate-400">Não participou</p>
          </div>
        </div>

        {/* Ganhas */}
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col justify-between border-b-4 border-b-green-500 hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start mb-2">
      ...
      {/* Área de Gráficos e Alertas */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Gráfico de Barras (Funil) - Ocupa 2 colunas */}
        <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <h2 className="text-lg font-semibold text-slate-900 mb-6">Status dos Processos</h2>
        ...
        {/* Gráfico de Pizza (Clientes) - Ocupa 1 coluna */}
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-red-100">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-bold text-slate-900">Atenção: Prazos Críticos (Próximas 72h)</h2>
        </div>

        {criticalDeadlines.length === 0 ? (
          <p className="text-slate-500 py-4 text-sm">Tudo tranquilo! Nenhuma licitação ativa vencendo agora.</p>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {criticalDeadlines.map(bid => (
                  <tr key={bid.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 font-medium text-slate-900">{bid.title}</td>
                    <td className="px-4 py-3 text-slate-600">{bid.client_name}</td>
