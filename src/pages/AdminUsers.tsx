...
const AdminUsers: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      toast.error(error.message || "Erro ao criar assessor.");
    }
  };

  const renderRoleBadge = (role: string) => {
    if (role === 'admin') return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-[#002A54]/10 text-[#002A54] border border-[#002A54]/20"><Shield size={10} /> ADMIN</span>;
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200"><Briefcase size={10} /> ASSESSOR</span>;
  };
  ...
  return (
    <div className="p-4 md:p-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#002A54] flex items-center gap-2">
            <Shield className="text-[#009B4D]" /> Gestão de Acessos
          </h1>
        ...
        <button 
          ...
          className="bg-[#009B4D] hover:bg-[#007A3D] text-white px-4 py-2 rounded-lg flex items-center gap-2 font-medium text-sm shadow-sm w-full md:w-auto justify-center"
        >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="overflow-x-auto">
                <div className="min-w-[600px]">
                    <div className="px-6 py-3 bg-slate-50 border-b border-slate-200 text-xs font-bold text-[#666666] uppercase flex">
                        ...
                        <div key={parent.id} className="border-b border-slate-100 last:border-0">
                            <div className={`flex items-center px-6 py-4 hover:bg-slate-50 transition-colors ${!parent.active ? 'bg-red-50/50' : ''}`}>
                                <div className="flex-1 flex items-center gap-3">
                                    <button onClick={() => toggleRow(parent.id)} className="text-slate-400 hover:text-[#002A54] p-1">
                                    ...
                                    <div>
                                        <div className="font-medium text-[#002A54] flex items-center gap-2">
                                            ...
                                            {parent.id === currentUserId && <span className="text-[9px] bg-slate-100 text-[#666666] px-1.5 rounded border border-slate-200">(Você)</span>}
                                            {!parent.active && <span className="text-[10px] bg-red-100 text-red-600 px-2 rounded-full font-bold">BLOQUEADO</span>}
                                        </div>
                                    </div>
                                </div>
                                
                                <div className="w-32 text-center text-sm text-[#666666] font-medium">
                                    {parent.clients.length} Clientes
                                </div>

                                <div className="w-24 text-center">
                                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${parent.active ? 'text-[#009B4D] bg-green-50' : 'text-red-700 bg-red-100'}`}>
                                ...
                                <div className="w-24 text-right">
                                    ...
                                    {parent.id !== currentUserId && (
                                      <button 
                                          ...
                                          className={`transition-colors p-2 rounded hover:bg-slate-200 ${parent.active ? 'text-[#009B4D]' : 'text-slate-400'}`}
                            ...
                            {expandedRows[parent.id] && (
                                <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 shadow-inner">
                                    <h4 className="text-xs font-bold text-[#666666] uppercase mb-3 ml-8">Clientes Vinculados</h4>
                                            ...
                                            {parent.clients.map(client => (
                                                <div key={client.id} className="flex justify-between items-center bg-white p-3 rounded border border-slate-200 hover:shadow-sm transition-shadow">
                                                    <div className="flex items-center gap-3">
                                                        <Globe size={16} className="text-[#002A54]"/>
                                                        <div>
                                                            <div className="text-sm font-bold text-[#002A54]">{client.companyName || 'Empresa (Nome não carregado)'}</div>
                                                            <div className="text-xs text-[#666666]">{client.email}</div>
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center gap-4">
                                                        <span className={`text-[10px] font-bold ${client.active ? 'text-[#009B4D]' : 'text-red-600'}`}>
                                                            {client.active ? 'PORTAL LIBERADO' : 'PORTAL BLOQUEADO'}
                                                        </span>
                                                        <button onClick={() => handleToggleSingleUser(client)} className="text-slate-400 hover:text-[#002A54]">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Cadastrar Novo Assessor">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input {...register('email', { required: true })} type="email" className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-[#002A54]" placeholder="assessor@email.com" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[#666666] mb-1">Senha Temporária</label>
            <input {...register('password', { required: true, minLength: 6 })} type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-[#002A54]" placeholder="Senha123" />
          </div>
          <div className="pt-4 flex justify-end gap-3">
            <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-[#666666] hover:bg-slate-100 rounded-lg">Cancelar</button>
            <button type="submit" className="px-4 py-2 bg-[#009B4D] hover:bg-[#007A3D] text-white rounded-lg">Cadastrar</button>
          </div>
        </form>
      </Modal>
    </div>
