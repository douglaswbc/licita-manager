    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button 
          ...
          className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 md:py-2 rounded-lg flex items-center justify-center gap-2 font-medium text-sm shadow-sm"
        >
                  ...
                  <td className="px-6 py-4 text-right">
                    {user.role !== 'admin' && (
                      <button 
                        ...
                        className={`transition-colors p-2 ${user.active ? 'text-green-600 hover:text-green-800' : 'text-slate-400 hover:text-slate-600'}`}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Cadastrar Novo Assessor">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="pt-4 flex flex-col-reverse md:flex-row justify-end gap-3">
            <button type="button" onClick={() => setIsModalOpen(false)} className="w-full md:w-auto px-4 py-2 text-slate-700 hover:bg-slate-100 rounded-lg">Cancelar</button>
            <button type="submit" className="w-full md:w-auto px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">Cadastrar</button>
          </div>
        </form>
      </Modal>
    </div>
