...
const DecisionBadge = ({ decision }: { decision?: string }) => {
  if (decision === 'Participar') {
    return <span className="flex items-center gap-1 text-[#009B4D] font-bold text-xs bg-green-50 px-2 py-1 rounded border border-green-100"><ThumbsUp size={12}/> Participar</span>;
  }
...
const Bids: React.FC = () => {
  ...
  const onSummarySubmit = async (data: { summary_link: string }) => {
    ...
    try {
      ...
      toast.update(idToast, { render: "Resumo enviado com sucesso!", type: "success", isLoading: false, autoClose: 3000 });
    } catch (e: any) {
      console.error(e);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-4 items-end md:items-center">
        ...
        <select value={filterClient} onChange={(e) => setFilterClient(e.target.value)} className="px-3 py-2 border border-slate-300 rounded-lg text-sm w-full md:w-auto outline-none text-[#666666]">
          <option value="">Todos os Clientes</option>
          {clients.map(c => <option key={c.id} value={c.id}>{c.company}</option>)}
        </select>

        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-3 py-2 border border-slate-300 rounded-lg text-sm w-full md:w-auto outline-none text-[#666666]">
          <option value="">Todos Status</option>
          {Object.values(BidStatus).map(s => <option key={s} value={s}>{s}</option>)}
        </select>

        <select value={filterDecision} onChange={(e) => setFilterDecision(e.target.value)} className="px-3 py-2 border border-slate-300 rounded-lg text-sm w-full md:w-auto outline-none text-[#666666]">
        ...
        <div className="ml-auto text-xs text-[#666666]">Encontrados: <strong>{filteredBids.length}</strong></div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-[#666666]">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-right">Resultado (R$)</th>
                <th className="px-6 py-4 font-semibold text-right">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredBids.length === 0 ? (
                <tr><td colSpan={7} className="px-6 py-8 text-center text-[#666666]">Nada encontrado.</td></tr>
              ) : (
                filteredBids.map((bid) => {
                  ...
                  return (
                    <tr key={bid.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-[#002A54] font-medium">{bid.title}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        {bid.final_value ? (
                          <div className="flex flex-col items-end">
                            <span className="font-bold text-[#666666]">R$ {bid.final_value.toLocaleString('pt-BR')}</span>
                            {commissionValue > 0 && (
                              <span className="text-[10px] text-[#009B4D] font-bold bg-green-50 px-1.5 py-0.5 rounded border border-green-100">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <button onClick={() => openSummaryModal(bid)} className={`p-2 rounded-lg transition-colors ${bid.summary_sent_at ? 'text-[#009B4D] bg-green-50' : 'text-slate-400 hover:text-purple-600 hover:bg-purple-50'}`}><Send size={16} /></button>
      ...
      <Modal isOpen={isFormModalOpen} onClose={() => setIsFormModalOpen(false)} title={editingBid ? 'Editar Licitação' : 'Nova Licitação'}>
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          <div>
            ...
            <input {...register('title', { required: true })} className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-[#002A54]" placeholder="Ex: Construção Ponte" />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-[#666666] mb-1">Link do Edital/Docs</label>
          ...
          <div className={`bg-green-50 p-4 rounded-lg border border-green-100 mt-2 transition-all duration-300 ${
            ...
            <h3 className="text-sm font-bold text-[#009B4D] mb-3 flex items-center gap-2">
              <Calculator size={16}/> Fechamento Financeiro
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-[#009B4D] mb-1">Valor da Nota/Licitação (R$)</label>
                <div className="relative">
                  <input 
                    {...register('final_value')}
                    type="number"
                    step="0.01"
                    className="w-full pl-8 px-3 py-2 border border-green-200 rounded-lg outline-none focus:ring-2 focus:ring-[#009B4D] bg-white"
                    placeholder="0,00"
                  />
                  <DollarSign className="absolute left-2.5 top-2.5 text-[#009B4D]" size={16} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-[#009B4D] mb-1">Taxa de Comissão (%)</label>
                <div className="relative">
                  <input 
                    ...
                    placeholder="0"
                  />
                  <Percent className="absolute left-2.5 top-2.5 text-[#009B4D]" size={16} />
                </div>
              </div>
            </div>
            ...
            {calculatedCommission > 0 && (
              <div className="mt-3 flex justify-between items-center bg-white px-3 py-2 rounded border border-green-100">
                <span className="text-xs text-[#009B4D]">Sua comissão estimada:</span>
                <span className="font-bold text-[#009B4D] text-lg">R$ {calculatedCommission.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span>
              </div>
            )}
          </div>
          ...
          <div className="pt-4 flex justify-end gap-3">
            ...
            <button type="submit" className="px-4 py-2 bg-[#009B4D] hover:bg-[#007A3D] text-white rounded-lg transition-colors font-medium">Salvar</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={isSummaryModalOpen} onClose={() => setIsSummaryModalOpen(false)} title="Link do Resumo">
        <form onSubmit={summaryForm.handleSubmit(onSummarySubmit)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#666666] mb-1">Link do Documento de Resumo</label>
            <div className="relative">
              <FileText className="absolute left-3 top-2.5 text-slate-400" size={18} />
              <input {...summaryForm.register('summary_link', { required: true })} className="w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-purple-500" placeholder="https://drive.google.com/..." />
            </div>
            <p className="text-xs text-slate-500 mt-1">Se deixar vazio, usarei o link do edital.</p>
          </div>
          <div className="pt-4 flex justify-end gap-3">
            <button type="button" onClick={() => setIsSummaryModalOpen(false)} className="px-4 py-2 text-[#666666] hover:bg-slate-100 rounded-lg">Cancelar</button>
            <button type="submit" className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg flex items-center gap-2"><Send size={16} /> Salvar e Enviar</button>
          </div>
        </form>
      </Modal>
    </div>
